...
class RetroBoardController {
    ...
    fun getCardByUUID(@PathVariable uuidCard: UUID): ResponseEntity<Card> {
        return ResponseEntity.ok<Card>(retroBoardService.findCardByUUID(uuidCard))
    }

    @PutMapping("/cards/{uuidCard}")
    fun updateCardByUUID(@PathVariable uuidCard: UUID, @RequestBody card: Card): ResponseEntity<Card> {
        val result: Card = retroBoardService.findCardByUUID(uuidCard)
        result.comment = card.comment
        return ResponseEntity.ofNullable(retroBoardService.saveCard(result))
    }

    @ResponseStatus(HttpStatus.NO_CONTENT)
    @DeleteMapping("/cards/{uuidCard}")
    fun deleteCardFromRetroBoard(@PathVariable uuidCard: UUID) {
    ...
    fun handleValidationExceptions(ex: MethodArgumentNotValidException): Map<String, Any> {
        val response: Map<String, Any> = mapOf(
            ...
            "time" to LocalDateTime.now().format(
                DateTimeFormatter.

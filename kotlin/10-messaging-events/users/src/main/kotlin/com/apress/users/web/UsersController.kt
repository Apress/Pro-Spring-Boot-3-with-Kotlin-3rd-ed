...
class UsersController {
    ...
    val all: ResponseEntity<Iterable<User>>
        get() = ResponseEntity.ok<Iterable<User>>(userService.allUsers)

    @GetMapping("/{email}")
    @Throws(Throwable::class)
    fun findUserById(@PathVariable email: String): ResponseEntity<User> {
        return ResponseEntity.of<User>(userService.findUserByEmail(email))
    }
    ...
    fun save(@RequestBody user: User): ResponseEntity<User> {
        ...
        val location: URI = ServletUriComponentsBuilder
            ...
            .toUri()
        return ResponseEntity.created(location).body<User>(userResult)
    }

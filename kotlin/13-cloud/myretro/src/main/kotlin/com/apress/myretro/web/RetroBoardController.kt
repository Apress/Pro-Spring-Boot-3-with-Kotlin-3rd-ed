...
class RetroBoardController {
    ...
    fun findRetroBoardById(@PathVariable uuid: UUID): ResponseEntity<RetroBoard> {
        return ResponseEntity.ok<RetroBoard>(retroBoardAndCardService.findRetroBoardById(uuid))
    }
    ...
    fun saveRetroBoard(
        ...
        return ResponseEntity.created(uri).body<RetroBoard>(saveOrUpdateRetroBoard)
    }
    ...
    val allUsers: ResponseEntity<*>
        // External Call
        get() {
            retroBoardAndCardService.allUsers()
            return ResponseEntity.ok(
                Map.of(
                    "status",
                    200,
                    "message",
                    "Users retrieved. Should be in the console",
                    "time",
                    LocalDateTime.now()
                )
            )
